...
async function displayAlbum() {
    ...
    Array.from(document.getElementsByClassName("card")).forEach(e => {
        e.addEventListener("click", async item => {
            let heading = e.getAttribute('data-folder');

            songs = await getSongs(`songs/${item.currentTarget.dataset.folder}`)

            document.querySelector("h3").innerHTML = heading.charAt(0).toUpperCase() + heading.slice(1);
...
async function main() {
    ...
    await getSongs("songs/ringtone-1")
    // console.log(songs);

    playMusic(songs[0])    //keep first song on track to play
